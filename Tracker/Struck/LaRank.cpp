...
LaRank::LaRank(const LaRank& other):	m_config(other.m_config),
                                    	m_features(*(const_cast<Features&>(other.m_features).clone())),
                                    	m_kernel(*(const_cast<Kernel&>(other.m_kernel).clone())),
                                    	m_debugImage(other.m_debugImage),
                                    	m_C(other.m_C),
                                    	m_K(other.m_K)
{
	for (SupportPattern* t : other.m_sps)
	{
		SupportPattern *mirrorT = new SupportPattern(*t);
		m_sps.push_back(mirrorT);
	}
	for (SupportVector* t : other.m_svs)
	{
		SupportVector *mirrorT = new SupportVector(*t);
		for (size_t i = 0; i < other.m_sps.size();++i)
		{
			if (t->x==other.m_sps[i])
			{
				mirrorT->x = m_sps[i];
			}
		}
		m_svs.push_back(mirrorT);
	}
}
...
LaRank::~LaRank()
{
	for (SupportPattern* t : m_sps)
	{
		delete t;
	}
	for (SupportVector* t : m_svs)
	{
		delete t;
	}
}
