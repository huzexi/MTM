...
UpdateInfo Tracker::TrackWithoutUpdate(const cv::Mat& frame)
{
	// TODO: Under Construction.
	assert(m_initialised);

	ImageRep *image = new ImageRep(frame, m_needsIntegralImage, m_needsIntegralHist);
	...
	return UpdateInfo(bestInd != -1, image);
}

Tracker* Tracker::clone()
{
	Tracker* mirrorTracker = new Tracker(*this);
	for (std::vector<Features*>::const_iterator it = m_features.begin();
		it != m_features.end();
		++it)
	{
		mirrorTracker->m_features.push_back(*it);
	}
